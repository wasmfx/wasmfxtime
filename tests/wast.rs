...
fn should_fail(test: &Path, wast_config: &WastConfig, test_config: &TestConfig) -> bool {
    ...
    if wast_config.strategy == Strategy::Winch {
        ...
        if test_config.gc == Some(true)
            ...
            || test_config.stack_switching == Some(true)
        {
            return true;
        }
        ...
        // TODO(dhil): Temporary hack as Winch do not yet have support
        // for stack switching and exceptions.
        if let Some(parent) = test.parent() {
            if parent.ends_with("stack-switching") || parent.ends_with("exception-handling") {
                return true;
            }
        }
    }
...
fn run_wast(wast: &Path, config: WastConfig) -> anyhow::Result<()> {
    ...
    if wast.starts_with("tests/misc_testsuite/stack-switching") {
    ...
    // internal panics in Winch due to partial implementation status.
    if !is_cranelift {
        if let Some(parent) = wast.parent() {
            if parent.ends_with("exception-handling") {
                return Ok(());
            }
        }
    }
