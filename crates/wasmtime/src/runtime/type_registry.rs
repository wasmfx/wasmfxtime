...
use wasmtime_environ::{
    ...
    TypeTrace, VMSharedTypeIndex, WasmDefType, WasmRecGroup,
};
...
impl RegisteredType {
    ...
    pub fn new(engine: &Engine, ty: WasmDefType) -> RegisteredType {
        let (entry, index, ty) = {
...
impl TypeRegistryInner {
    ...
    fn register_rec_group(
        ...
        types: impl ExactSizeIterator<Item = WasmDefType>,
    ) -> RecGroupEntry {
    ...
    fn assert_canonicalized_for_runtime_usage_in_this_registry(&self, ty: &WasmDefType) {
    ...
    /// The returned entry will have already had its reference count incremented
    /// on behalf of callers.
    fn register_singleton_rec_group(&mut self, ty: WasmDefType) -> RecGroupEntry {
...
impl TypeRegistry {
    ...
    pub fn borrow(&self, index: VMSharedTypeIndex) -> Option<Arc<WasmDefType>> {
        let id = shared_type_index_to_slab_id(index);
